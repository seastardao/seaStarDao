...
contract S5NFTPOOL is IERC721Receiver {
    enum opreate {
        stake,
        upStake,
        claim
    ...
    constructor(address _ssd, address _starNodeData) {
        owner = msg.sender;
        ssd = _ssd;
        lock = address(new TokenLock(_ssd));
        IERC20(ssd).approve(lock, type(uint).max);
        s5Nft = address(new S5NFT(_starNodeData));
    }
    function mint(address _user) external {
        S5NFT(s5Nft).mint(_user);
    }
    ...
    function _pledgeNft(address _sender, uint _tokenid) internal {
        ...
        IERC721(s5Nft).transferFrom(_sender, address(this), _tokenid);
    }
    ...
    function _unpack(address sender, uint _tokenid) private {
        ...
        IERC721(s5Nft).safeTransferFrom(address(this), sender, _tokenid);
