...
import {IAsdicLpPool} from "./interface/IAsdicLpPool.sol";
import {TokenLock} from "./lock.sol";

contract ASDICLPSSDPOOL {
    enum opreate {
        stake,
        upStake,
        claim
    ...
    IERC20 public ssd;
    address public lock;
    IAsdicLpPool public asdicLpPool;
    ...
    function setAddress(address _asdicLpPool, address _ssd) external {
        require(msg.sender == owner, "Not owner");
        ssd = IERC20(_ssd);
        lock = address(new TokenLock(_ssd));
        asdicLpPool = IAsdicLpPool(_asdicLpPool);
    ...
    function sync() external {
        ...
        uint newAmount = IAsdicLpPool(asdicLpPool)
            .userStakedInfos(sender)
            .stakedAmount;
        uint temp;
