...
contract SSD is ERC20, Ownable {
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        swapTokensForEth(half, mos);
        swapTokensForEth(otherHalf / 2, usdt);
    ...
    function swapTokensForEth(uint256 tokenAmount, address tokenType) private {
        if (tokenType == mos) {
            ...
            safeTransferFrom(
                ...
                tokenReceiver,
                address(this),
                IERC20(usdt).balanceOf(tokenReceiver)
            );
        }
    }
